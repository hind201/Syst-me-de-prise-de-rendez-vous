...
const CreateRdv = () => {
  ...
  const getPlaning = async () => {
    try {
      const { data } = await axios.get('http://172.16.11.172:3000/date/getplaning');
      setPlaning(data);
      console.log(data)
  ...
  const sendData = async () => {
    fetch('http://172.16.11.172:3000/date/rdv', {
      method: 'POST',
      mode: 'no-cors',
      headers: {
  ...
  return (
    < View  style={styles.body}> 
      <Text style={{ textAlign: 'center', fontSize: 20,color:'#822659'}}>Rendez-vous</Text>
      <SafeAreaView>
        <Picker
